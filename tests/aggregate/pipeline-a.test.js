...
describe('runPipelineA', () => {
  it('fetches, parses, upserts one 13F entry', async () => {
    ...
    nock('https://www.sec.gov')
      .get('/Archives/edgar/data/1067983/000106798326000123/form13fInfoTable.xml')
      .reply(200, readFileSync(join(import.meta.dirname, '../fixtures/form13fData.xml'), 'utf8'));
    const r = await runPipelineA({
      httpClient,
      config,
      feedPath: join(dir, 'feed-13f.json'),
      statePath: join(dir, 'state-13f.json'),
    });
  ...
  it('skips already-seen accession (state dedup)', async () => {
    ...
    writeStateJson(join(dir, 'state-13f.json'), {
      lastUpdated: 'x',
      seenFilings: { '0001067983-26-000123': 1 },
    });
    const r = await runPipelineA({
      httpClient,
      config,
  ...
  it('compares against prior period holdings of the same filer (regardless of CIK padding)', async () => {
    ...
    const xmlQ4 =
      '<?xml version="1.0"?><informationTable><infoTable><nameOfIssuer>APPLE INC</nameOfIssuer><cusip>037833100</cusip><value>1000</value><shrsOrPrnAmt><sshPrnamt>100</sshPrnamt></shrsOrPrnAmt><votingAuthority><Sole>100</Sole><Shared>0</Shared><None>0</None></votingAuthority></infoTable></informationTable>';
    const xmlQ1 =
      '<?xml version="1.0"?><informationTable><infoTable><nameOfIssuer>APPLE INC</nameOfIssuer><cusip>037833100</cusip><value>2000</value><shrsOrPrnAmt><sshPrnamt>200</sshPrnamt></shrsOrPrnAmt><votingAuthority><Sole>200</Sole><Shared>0</Shared><None>0</None></votingAuthority></infoTable><infoTable><nameOfIssuer>ALPHABET INC</nameOfIssuer><cusip>02079K305</cusip><value>3000</value><shrsOrPrnAmt><sshPrnamt>50</sshPrnamt></shrsOrPrnAmt><votingAuthority><Sole>50</Sole><Shared>0</Shared><None>0</None></votingAuthority></infoTable></informationTable>';
    ...
    nock('https://www.sec.gov')
      ...
      .reply(200, { directory: { item: [{ name: 'form13fInfoTable.xml', size: 5000 }] } });
    nock('https://www.sec.gov')
      .get('/Archives/edgar/data/1067983/000106798325999001/form13fInfoTable.xml')
      .reply(200, xmlQ4);
    nock('https://www.sec.gov')
      .get('/Archives/edgar/data/1067983/000106798326000123/index.json')
      .reply(200, { directory: { item: [{ name: 'form13fInfoTable.xml', size: 5000 }] } });
    ...
    const q1 = JSON.parse(readFileSync(join(dir, 'feed-13f.json'), 'utf8')).thirteenF.find(
      (e) => e.periodOfReport === '2026-03-31',
    );
  ...
  it('writes feed-13f.json exactly once per run (O(1), not per filing)', async () => {
    ...
    const xmlQ4 =
      '<?xml version="1.0"?><informationTable><infoTable><nameOfIssuer>APPLE INC</nameOfIssuer><cusip>037833100</cusip><value>1000</value><shrsOrPrnAmt><sshPrnamt>100</sshPrnamt></shrsOrPrnAmt><votingAuthority><Sole>100</Sole><Shared>0</Shared><None>0</None></votingAuthority></infoTable></informationTable>';
    const xmlQ1 =
      '<?xml version="1.0"?><informationTable><infoTable><nameOfIssuer>APPLE INC</nameOfIssuer><cusip>037833100</cusip><value>2000</value><shrsOrPrnAmt><sshPrnamt>200</sshPrnamt></shrsOrPrnAmt><votingAuthority><Sole>200</Sole><Shared>0</Shared><None>0</None></votingAuthority></infoTable></informationTable>';
    nock('https://www.sec.gov')
      .get('/Archives/edgar/data/1067983/000106798325999001/index.json')
      .reply(200, { directory: { item: [{ name: 'form13fInfoTable.xml', size: 5000 }] } });
    nock('https://www.sec.gov')
