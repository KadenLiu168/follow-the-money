  ...
  for (const formType of FORMS) {
    try {
      ...
      for (const h of hits) {
        ...
        let docUrl = `${baseUrl}/primary_doc.html`;
        try {
          const indexRes = await httpClient.fetch(`${baseUrl}/index.json`);
          if (indexRes.ok) {
            ...
            if (htmls.length > 0) {
              ...
              docUrl = `${baseUrl}/${htmls[0].name}`;
            }
          }
        } catch { /* fall through to primary_doc.html */ }
