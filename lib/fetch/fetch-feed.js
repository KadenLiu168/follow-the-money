...
// Generous timeout for SEC EDGAR raw file fetches — avoids premature aborts on
// slow GitHub raw responses without hanging indefinitely.
const DEFAULT_TIMEOUT_MS = 15000;
// Two retries with the backoff below covers transient 429/5xx without stalling
// the aggregate run on a single flaky request.
const DEFAULT_RETRIES = 2;
// Backoff between retries (ms): 500ms then 1500ms — long enough for EDGAR to
// recover from a transient rate-limit, short enough to keep the run snappy.
...
export async function fetchFeed({
  ...
  const ndjsonFiles = Object.values(manifest.years ?? {})
    .map((y) => y.file)
    .filter(Boolean);
  ...
  if (ndjsonFailures.length > 0) {
    ...
    console.warn(
      `[fetch-feed] missing NDJSON files: ${ndjsonFailures.map((f) => f.localName).join(', ')}`,
    );
  }
