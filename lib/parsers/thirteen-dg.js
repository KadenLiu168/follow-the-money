...
export function parseThirteenDG(html, { formType }) {
  ...
  const percentRaw = Number(pickFirst(html, ['PERCENT OF CLASS'], ['SHARED VOTING POWER', 'SOLE VOTING POWER', 'AGGREGATE AMOUNT BENEFICIALLY OWNED']) || '0');
  const percent = Number.isFinite(percentRaw) ? percentRaw : 0;
  const sharesRaw = Number((pickFirst(html, ['AGGREGATE AMOUNT BENEFICIALLY OWNED']) || '0').replace(/,/g, ''));
