...
function yearFile(feedDir, year) {
  return join(feedDir, `${year}.ndjson`);
}
...
export function append13DFiling(feedDir, manifest, entry) {
  ...
  if (year === null) {
    console.warn(
      `[feed-ndjson] skipping entry with invalid filingDate: ${JSON.stringify(entry.filingDate)}`,
    );
    return manifest;
  }
  ...
  const y = manifest.years[year] ?? {
    file: `feed-13dg/${year}.ndjson`,
    count: 0,
    firstDate: entry.filingDate,
    lastDate: entry.filingDate,
  };
  y.count = (y.count ?? 0) + 1;
  y.firstDate =
    entry.filingDate < (y.firstDate ?? entry.filingDate) ? entry.filingDate : y.firstDate;
...
export function validateManifest(feedDir, manifest) {
  ...
  for (const [year, meta] of Object.entries(manifest.years ?? {})) {
    const file = yearFile(feedDir, year);
    if (!existsSync(file)) {
      warnings.push(`${year}: file missing`);
      continue;
    }
    let actual = 0,
      corrupt = 0;
    ...
    if (actual !== meta.count)
      warnings.push(`${year}: manifest says ${meta.count}, file has ${actual}`);
