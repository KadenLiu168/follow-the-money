...
The eight managers tracked are: Berkshire Hathaway, Bridgewater Associates, Renaissance Technologies, Citadel Advisors, Tiger Global Management, Pershing Square Capital, Baupost Group, and Soros Fund Management. Filings are pulled from EDGAR's public dataset; the skill never scrapes paywalled sources and never stores credentials beyond the optional Telegram bot token and SMTP password stored locally in `~/.follow-the-money/config.json`.
