...
## Purpose
Define the implemented minimum private Host-Agent process boundary for invoking selected deterministic Skill operations while preserving compatibility, failure, ownership, provenance, and bounded-authority semantics.
...
- **WHEN** a request explicitly supplies an integer `contract_version` other than `1`
