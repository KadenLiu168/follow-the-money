...
This capability SHALL define semantic grounding, validation-authority, admissibility, unsupported-assertion, and recovery rules only. It SHALL NOT introduce or require `ResearchContext`, `AgentAnalysis`, `BriefContext`, equivalent Agent DTOs, Agent-facing Claim objects, Agent-facing Claim JSON Schema, final-output JSON Schema, Agent audit schema, serialized Agent request or response contracts, MCP/RPC/HTTP interfaces, a validator service or facade, an Agent runtime adapter, mandatory `ClaimAuditor` invocation, automatic semantic or citation-entailment validation, Feed-to-retained-library wiring, retained-capability production wiring, Agent orchestration, a fixed Agent or Brief pipeline, invocation order or counts, retry counts, rewrite loops, shared runtime state, prompt architecture, model SDK or configuration, API-key configuration, an embedded LLM runtime, or Phase-5 implementation architecture. Existing internal audit structures SHALL remain internal and SHALL NOT become the Host-Agent contract.

#### Scenario: Serialized contracts are inspected
- **WHEN** this capability and the repository's current serialized schemas are reviewed
- **THEN** no Agent-facing, Claim, audit, or final-output schema has been introduced and `feed.schema.json` remains the only current serialized external contract
...
- **WHEN** existing `AuditClaim`, `AuditFlow`, `AuditResult`, or related internal Python structures are reviewed against this contract
- **THEN** they remain internal capability inputs or results and are not promoted into an Agent-facing semantic or serialized contract
...
- **THEN** this contract defines no such capability and does not claim that the current deterministic engine provides it
