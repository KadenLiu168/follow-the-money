...
- [x] 4.1 After explicit authorization for the external repository mutation, trigger the hosted Feed workflow and verify product-only migration publishes the exact generated-state commit, deletes `feeds/latest.json`, performs zero Provider requests, and exits before collection.
  - Run `33719280532` succeeded on `5bc83b4`; generated commit `231d4f90d6e6972d512e2c435fba148e0cb04530` added the manifest and 8 artifacts, removed `feeds/latest.json`, and skipped collection.
- [x] 4.2 After migration completion and any existing recovery boundary, trigger the next hosted invocation and record whether normal arming and collection execute; hand that evidence to the separate `align-provider-runtime-contract-and-html-parsing` Change without modifying or completing its verification as part of this Change, and do not treat external Provider success as unconditional.
  - Run `33719354209` on migration commit `231d4f90`: arming and pre-network publication succeeded; collection executed and failed with upstream `HTTP 403` from BLS/SSE and a `ConnectError` from SZSE, while other providers returned empty/healthy outcomes. Finalization preserved the failure in commit `a9358a4`. This evidence is handed off to `align-provider-runtime-contract-and-html-parsing`; its verification was not modified or completed here.
