...
## Purpose
Ensure the feed directory is resolved by a single source of truth, and that in skill mode the fetch step and the consumer steps (prepare / check-alerts) resolve to the **same** directory — so freshly fetched data is actually read instead of silently bypassed.
