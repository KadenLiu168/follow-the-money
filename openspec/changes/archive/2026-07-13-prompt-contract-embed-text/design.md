...
- 中心 prompt 改进自动传播(无需 git pull);离线降级到 clone,离线兜底无回归(无 GitHub 时 digest 仍可生成)。
...
无需 import。以下任一情况 → 静默 fall through 到 clone 层;resolver **永不因远程/网络错误抛错**
(每个 name 的远程尝试包在 try/catch,异常被吞掉,保证 `prepare-digest.js` 的顶层 `await`
不会因网络瞬断 reject——否则本可离线降级却直接崩溃,降低 digest 可用性):
...
- 本地 prompt 文件读失败(用户 / 仓库文件权限异常或 corrupt)仍会向上抛出,由顶层 `await` reject
  触发非零退出、无 stdout,属符合「铁律」(部分输出比无输出更糟)的干净硬失败,**不**在此吞掉;
  远程层吞错仅为可用性兜底,不绕过铁律。

离线兜底:clone 必存在(仓库已 checkout),故始终可降级,无回归。
...
- [Risk] 远程 URL 用 `node:path.join(remoteBaseUrl, name)` 拼接,仅在 POSIX(macOS / Linux)产生 `/`
  分隔符;Windows 上会生成反斜杠 URL(`.../prompts\format-13f.md`),`fetch` 失败并静默降级到 clone——
  远程「新鲜度」层在 Windows 上实际永不生效(非崩溃,但功能不可用)。
  → Mitigation:当前仅 macOS / Linux 目标(用户态 `node scripts/*`,`engines: ">=20.19.0"`),可接受;
  若未来需 Windows 支持,改用 `new URL(remoteBaseUrl)` + `new URL(name, base)` 或显式 `/` 拼接,
  勿依赖 `node:path` 拼 URL(见 D2 URL 构造)。
